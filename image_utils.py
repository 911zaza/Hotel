...
def save_plat_image(upload_file: UploadFile) -> str:
    """
    Sauvegarder une image de plat et retourner son URL
    """
    if not is_allowed_file(upload_file.filename):
        raise HTTPException(
            status_code=400,
    ...
    try:
        # Créer un nom de fichier unique
        filename = f"plat_{Path(upload_file.filename).stem}_{int(os.urandom(4).hex(), 16)}{Path(upload_file.filename).suffix}"
        filepath = os.path.join(PLAT_IMAGES_DIR, filename)
        ...
        # Construire l'URL publique en utilisant la base configurée (ou fallback)
        api_base = os.environ.get("API_BASE", "http://127.0.0.1:9090").rstrip("/")
        return f"{api_base}/images/plats/{filename}"
...
def delete_image_file(image_url: str) -> None:
    ...
    try:
        ...
        if "/images/rooms/" in image_url:
            ...
            filepath = os.path.join(EVENT_IMAGES_DIR, filename)
        elif "/images/plats/" in image_url:
            filename = image_url.split("/images/plats/")[-1]
            filepath = os.path.join(PLAT_IMAGES_DIR, filename)
        else:
            return
